...
searchPerson queryText =
    let
        url =
            Http.url apiBaseUrl [ ( "api_key", apiKey ), ( "query", queryText ) ]
    in
...
decodeActor =
    Json.Decode.object3 Actor decodeName decodeId decodeImage
...
decodeImage : Decoder (Maybe String)
decodeImage =
    maybe ("profile_path" := Json.Decode.string)
